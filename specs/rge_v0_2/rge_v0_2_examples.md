...
`state_vector` is the eight-axis governance state used by the Phionyx runtime. `time_delta = 0.1` flags this as the first turn in the trace. `safety` carries the input-safety-gate verdict; `reason` is NULL when admitted, a string when blocked (e.g. `"blocked patterns: ['ignore previous instructions']"`).

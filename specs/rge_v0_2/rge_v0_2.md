...
The Phionyx governance wrapper emits a per-turn envelope that records (a) the runtime decision, (b) the pipeline-block path taken, (c) numeric coherence metrics, and (d) a hash-chained integrity record. We refer to this as the **Reasoned Governance Envelope (RGE)**. v0.1 was implicit in the wrapper implementation; this RFC fixes it as a versioned schema and extends it for four 2026 trends:
...
Multi-agent handoff chain. `status` distinguishes reserved (`reserved-for-v0.6.0-f5`) from active.

Required keys: `agent_id`, `role` ∈ {root, child, leaf}.

Optional keys: `parent_envelope_hash` (NULL when role=root), `handoff_signature` (NULL when role=root), `protocol` ∈ {a2a, agntcy, phionyx_native, langgraph_subgraph, crewai, autogen}.

F5 ("Subagent / multi-agent audit chain") delivers populating logic in v0.6.0.

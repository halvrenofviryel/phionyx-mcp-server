...
`phionyx-mcp-server` is an MCP trust boundary — descriptor hashing, signed RGE v0.2 envelopes, and an audit chain over third-party MCP tool calls. It is **alpha / experimental** software. Its value depends on its
...
**founder@phionyx.ai** with `[SECURITY] phionyx-mcp-server` in the subject. Please do not open a public
issue or PR for a suspected vulnerability until it has been triaged.

Include, where you can: the version (`pip show phionyx-mcp-server`), a minimal reproduction, and what
